...
export default function mpa(userOptions: UserOptions = {}): Plugin {
  ...
  return {
    ...
    config(config) {
      ...
      options.base = config.base || options.base; // Fix the issue with options.base assignment
      // default '' means first-page and you can customized or disabled.
      config.server.open =
        options.open === '' ? getFirstPage(config.build.rollupOptions.input, options.base) : options.open
    },
    ...
    closeBundle() {
      ...
      // 3. copy src/pages/* to dest root if it exists
      if (shell.test('-d', resolve(`${dest}/${options.scanDir}`))) {
        shell.cp('-Rf', resolve(`${dest}/${options.scanDir}/*`), resolve(dest))
      }
